...
window.onload = function () {
  ...
  const initialNight = today.getHours() < 6 || today.getHours() >= 18;
  document.body.dataset.theme = getThemeFromCondition("clear", initialNight);
...
function displayWeather(data) {
  ...
  const isNight = icon.endsWith("n") || localHour < 6 || localHour >= 18;

  document.body.dataset.theme = getThemeFromCondition(
    mainCondition,
    isNight,
    temp,
  );
...
function getThemeFromCondition(condition, isNight, temp) {
  ...
  if (
    ...
    return isNight ? "mist-night" : "mist-day";
  }
  ...
  if (typeof temp === "number" && temp >= 34) return "default-day";
